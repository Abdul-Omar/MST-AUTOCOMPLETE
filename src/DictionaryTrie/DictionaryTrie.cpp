  ...
  //if equal
	...
	if((*node)->wordEnd) return false;
	  ...
	  //otherwise
          (*node)->freq = freq;
          (*node)->wordEnd = true;
	  if(root->middle  == nullptr && root->right == nullptr && root->left == nullptr)
	   root->maxBelow  = freq;
...
* Description:*Parameters:
            ...
            prefix - the prefix whose completions to search for
            numCompletions - the number of search results
	    pq-holds requireed number of completions
	    numCompletions = required  number of completions
...
/*
* Function Name: getWildCard()
* Function Prototype: getWildCard(TrieNode* root, string wildCard, string prefix, vector<pair<string, unsigned int>> & vect, unsigned int index) 
* Description:*Parameters:
            root- the root of the tree
	    wilcard -string containing wildcards
            prefix - the prefix whose completions to search for
            numCompletions - the number of search results
	    vect-holds requireed number of completions
	    numCompletions = required  number of completions
